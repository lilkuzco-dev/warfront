...
//   - HARD GATE: build fails if player options < 1000
...
const CONDITION_KEYS = new Set(["faction", "standing", "disposition", "role", "location", "time",
	"recent_combat", "has_killed_this_faction", "active_contract", "tech_level_min", "tech_level_max",
	"requires_item", "requires_count"]);
const OPTION_KEYS = new Set(["id", "text", "response", "conditions", "effects", "weight", "once_per",
	"cooldown_minutes", "exit"]);
...
const EVENT_TYPES = new Set(["attacked_soldier", "killed_soldier", "destroyed_property", "trespassed",
	...
	"apology_tribute_paid", "penance_completed", "insulted", "bribed"]);
...
for (const file of fs.readdirSync(SRC).filter((f) => f.endsWith(".json")).sort()) {
	...
	for (const option of src.options ?? []) {
		...
		const norm = normalize(option.text);
...
const optionTexts = options.map((o) => ({ id: o.id, tok: tokens(o.text), len: tokens(o.text).size }));
...
if (options.length < 1000) {
	errors.push(`HARD COUNT GATE: ${options.length} player options < 1000`);
}
if (errors.length) {
