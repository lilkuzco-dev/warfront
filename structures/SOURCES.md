...
| `strongholds/end/library_big.nbt` | Grand-castle library core |
| `strongholds/end/library_small.nbt` | Grand-castle scientific/alchemical study core |
| `strongholds/end/chest_corridor.nbt` | Concealed castle vault passages |
...
## Grand-castle composition

The central builds in the three normal 501×501 faction castle assets are imported
from the user-supplied **Celestial Castle**, **Cinderella's Armored Castle**, and
**Mug Castle** worlds. Creator, source, and permission details are in `CREDITS.md`.
`tools/import-grand-castle.js` preserves source block states and block-entity details,
removes downloaded inventories, assigns bounded faction loot, and embeds a persistent
king and faction guard complement.

The four surrounding working-town districts are original Warfront layouts released
under LGPL-3.0-or-later. Their frozen pre-import shells live in
`structures/working-town-shells/`; the importer copies only those outer districts and
their connecting roads around the source castle. They supply physical housing, farms,
worksites, markets, storage, and garrison bunks for the castle economy. Reproduction
commands and the faction/source mapping are in `structures/CASTLE-IMPORTS.md`.

The separate Dracula castle structure is imported, with authorization, from **Creepy
Blackstone Castle | Halloween Edition** by Nevas Buildings (builders Jatos, Nebaj and
Nasdas). Its checked-in NBT is produced by `tools/import-dracula-castle.js`; it is not
regenerated by `tools/gen-base-plans.js`.
